...
@router.get("/read/{class_id}", response_model=ClassOut)
...
@router.delete("/desactivate/{class_id}", response_model=ClassOut)
def deactivate_class(class_id: int, db: Session = Depends(get_db)):
    deactivated = class_crud.deactivate_class(db, class_id)
    if not deactivated:
        raise HTTPException(status_code=404, detail="Class not found")
    return deactivated